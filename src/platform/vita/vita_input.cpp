    ...
    if (event != NULL) {
        ...
        case SDL_EVENT_JOYSTICK_BUTTON_DOWN:
            ...
            case VITA_PAD_RIGHT: // intentional fallthrough
                vita_button_to_sdlkey_event(event->jbutton.button, event, SDL_EVENT_KEY_DOWN);
                break;
            case VITA_PAD_CROSS:
            case VITA_PAD_R:
            case VITA_PAD_CIRCLE:
            case VITA_PAD_L: // intentional fallthrough
                vita_button_to_sdlmouse_event(event->jbutton.button, event, SDL_EVENT_MOUSE_BUTTON_DOWN);
                break;
            case VITA_PAD_START:
                vkbd.requested = 1;
                break;
        ...
        case SDL_EVENT_JOYSTICK_BUTTON_UP:
            ...
            case VITA_PAD_LEFT:
            case VITA_PAD_RIGHT: // intentional fallthrough
                vita_button_to_sdlkey_event(event->jbutton.button, event, SDL_EVENT_KEY_UP);
                break;
            case VITA_PAD_CROSS:
            case VITA_PAD_R:
            case VITA_PAD_CIRCLE:
            case VITA_PAD_L: // intentional fallthrough
                vita_button_to_sdlmouse_event(event->jbutton.button, event, SDL_EVENT_MOUSE_BUTTON_UP);
                break;
            case VITA_PAD_SELECT:
...
static void vita_start_text_input(void) {
    ...
    for (int i = 0; i < MAX_VKBD_TEXT_SIZE; i++) {
        vita_create_and_push_sdlkey_event(SDL_EVENT_KEY_DOWN, SDL_SCANCODE_BACKSPACE, SDLK_BACKSPACE);
        vita_create_and_push_sdlkey_event(SDL_EVENT_KEY_UP, SDL_SCANCODE_BACKSPACE, SDLK_BACKSPACE);
    }
    for (int i = 0; i < MAX_VKBD_TEXT_SIZE; i++) {
        vita_create_and_push_sdlkey_event(SDL_EVENT_KEY_DOWN, SDL_SCANCODE_DELETE, SDLK_DELETE);
        vita_create_and_push_sdlkey_event(SDL_EVENT_KEY_UP, SDL_SCANCODE_DELETE, SDLK_DELETE);
    }
